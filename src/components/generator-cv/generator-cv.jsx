...
const styles = StyleSheet.create({
  ...
  row: { flexDirection: "row", borderBottom: "1px solid black", display: 'flex', minHeight: 20, width: '100%' },
  cellHeader: {
    fontSize: 10,
    fontWeight: "bold",
    display: 'flex',
  ...
  cell: { fontSize: 10, width: "60%", display: 'flex', alignItems: 'center', justifyContent: 'center' },
  cellItems: { width: "60%" },
  cellNumber: { display: 'flex', alignItems: 'center', justifyContent: 'center', width: '5%', heigth: '100%' },
  title: { 
    fontSize: 12,
    textAlign: "center",
    marginBottom: 10,
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.table}>
          ...
          <View style={styles.row}>
            ...
            <View style={styles.cellHeader}>
              <Text style={styles.subtitle}>Date et Lieu de Naissance (Date and Place of birth)</Text>
            </View>
            <View style={styles.cell}>
              <Text style={styles.simpleText}>{fDate(userData.birthDate)} à {userData.birthPlace}</Text>
            </View>
          </View>
          ...
          <View style={styles.row}>
            ...
            <View style={styles.cellHeader}>
              <Text style={styles.subtitle}>Statut Matrimonial et Nombre d&apos;enfants (Marital Status)</Text>
            </View>
            <View style={styles.cell}>
          ...
          <View style={styles.row}>
            ...
            <View style={styles.cellHeader}>
              <Text style={styles.subtitle}>Expérience professionnelle (Professional experience)</Text>
            </View>
            <View style={styles.cellItems}>
              <View style={styles.row}>
                <View style={{flex: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRight: "1px solid black", paddingHorizontal: 15}}>
                  <Text style={styles.subtitle}>SOCIETES (Compagnies)</Text>
                </View>
                <View style={{flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRight: "1px solid black", paddingHorizontal: 15}}>
                  <Text style={styles.subtitle}>POSITIONS (Position)</Text>
                </View>
                <View style={{flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', paddingHorizontal: 15}}>
                  <Text style={styles.subtitle}>PERIODES (Period)</Text>
                </View>
              </View>
              {filledExperiences.map((exp, index) => (
                <View key={`exp-${index}`} style={styles.row}>
                  <View style={{flex: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRight: "1px solid black", paddingHorizontal: 5,}}>
                    <Text style={styles.simpleText}>{exp.company}</Text>
                  </View>
                  <View style={{flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRight: "1px solid black", paddingHorizontal: 5}}>
                    <Text style={styles.simpleText}>{exp.position}</Text>
                  </View>
                  <View style={{flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center',paddingHorizontal: 5}}>
                    {
                      exp.begin_at.length > 0 ?
                      <Text style={styles.simpleText}>{fDate(exp.begin_at)} - {exp.end_at ? fDate(exp.end_at) : "En cours"}</Text>
                      :
                      <Text />
                    }
                  </View>
          ...
          <View style={styles.row}>
            ...
            <View style={styles.cell}>
              <Text style={styles.simpleText} >{degreeAccademy.find(degreeItem => parseInt(degreeItem.value.split(':')[1], 10) === userData.levelGruaduate).label}</Text>
            </View>
          </View>
        ...
        <View style={{marginTop: 5}}>
          <Text style={{...styles.title, fontSize: 10, textDecoration: 'underline', textAlign: 'left'}}>For the languages</Text>
        ...
        <View style={{marginTop: 5}}>
          <Text style={{...styles.title, fontSize: 10, textDecoration: 'underline', textAlign: 'left'}}>Pour les langues</Text>
          <View style={{display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between'}}>
