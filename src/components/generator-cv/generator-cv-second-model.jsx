      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={{...styles.row, ...styles.spaceBetween}}>
          <View style={styles.boxDashed}>
            ...
            <Text style={styles.subtitle}>Date et lieu de naissance: {fDate(userData.birthDate)} à {userData.birthPlace}</Text>
          ...
          <Image src={{ uri: `https://api.icmemployment.net/api/v1/admin/profilPicture/${userData.profil}`, method: "GET", headers:{'Access-Control-Allow-Origin': '*', "Cache-Control": "no-cache", 'Accept': '*/*'}}} style={styles.profileImage} />
            <Image
              src={logo}
              style={styles.logo}
            />
        ...
        <View style={{...styles.row, ...styles.center, width: '100%', marginTop: 2}}>
          {userData.domainsActivity.map((domaine, index) => (<Text style={styles.simpleText} key={`domaine_${domaine}`}>{domaine}{userData.domainsActivity.length > index + 1 && '/ '}</Text>))}
        </View>
        ...
        <View style={{...styles.row, width: '100%', gap: 15}}>
          <View style={styles.fistDisposation}>
            ...
            <Text style={styles.title}>{degreeAccademy.find(degreeItem => parseInt(degreeItem.value.split(':')[1], 10) === userData.levelGruaduate).label}</Text>
            <View style={styles.section}>
              <Text style={{...styles.title, color: blueColors}}>Langues (Languages)</Text>
            </View>
              {userData.languages.map(
                        ({title, written, spoken}, index) => (
                          <View key={`language-${index}`} style={{...styles.row, alignItems: 'center', justifyContent: 'start'}}>
                      ...
                      )}
                      {
                        userData.languages.length > 0 && (
                          <View style={{display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderTopWidth: 1, borderTopColor: 'black', borderTopStyle: 'solid', paddingTop: 3}}>
          ...
          <View style={styles.secondDisposation}>
            ...
            {filledCertifications.map(({certification, expire_at}, index) => (
              <View key={`certification-${index}`} style={{...styles.row, alignItems: 'center', justifyContent: 'start'}}>
                <Image src={etiquette} style={styles.tag} />
            ...
            <View style={styles.section}>
              <Text style={{...styles.title, color: blueColors}}>Expérience professionnelle (Professional experience)</Text>
            </View>
            {filledExperiences.map((exp, index) => (
              <View key={`filledExperiences-${index}`} style={{...styles.row, flexWrap: 'wrap', alignItems: 'start', justifyContent: 'start' }}>
                <Image src={etiquette} style={{...styles.tag, marginTop: 5}} />
                <Text style={styles.subtitle}>{fDate(exp.begin_at)} - {exp.end_at ? fDate(exp.end_at) : "En cours"} :{' '}</Text>
                <Text style={{...styles.simpleText, fontSize: 12 }}>À {exp.company} {' '}</Text>
                <Text style={{...styles.simpleText, fontSize: 12 }}>en tant que {exp.position}</Text>
              </View>
            ))}
          </View>
        </View>
