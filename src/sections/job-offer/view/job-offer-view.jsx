  ...
  const columns = [
    {
        ...
        render: (_, company) => (
            <Stack sx={{maxWidth: 200}} alignItems='center' justifyContent='center' key={`domaine-${company.id}`}>
    ...
    <Menu>
      ...
      </Menu.Item>
      {
          role === RoleEnum.ADMIN_PARTENAIRE && (
            <Menu.Item key={`updateConfigItem${_id}`} onClick={() => handleActionEditConfig(_id, config)}>
                <Flex gap='middle' align='center' justify='space-between'>
      ...
      }
      {
        ...
        isActive ?
          (
          <Menu.Item className='hover-danger' key={`removeItem${_id}`} onClick={() => handleActionJob(_id, false)}>
            <Flex gap='middle' align='center' justify='space-between'>
        ...
        <>
          ...
          <Dialog maxWidth="xl" fullWidth disableEscapeKeyDown open={openCreatePartner} onClose={handleToogleDialogCreatePartners}>
            ...
            <DialogContent>
              ...
              <Grid container spacing={2}>
                <Grid size={{ xs: 12, sm: 12, md: 12 }}>
                  <Typography>
                    Veuillez sélectionner une photo de profil pour le compte
                  </Typography>
            ...
            <DialogActions>
              ...
              <Button variant="contained" onClick={createPartners} disabled={isCreating} startIcon={isCreating && <AutorenewIcon />} >{isCreating ? "Enregistrement": 'Enregistrer'}</Button>
            </DialogActions>
          </Dialog>


          <Dialog maxWidth='xl' fullWidth disableEscapeKeyDown open={openCreateOrUpdateConfig} onClose={handleToogleDialogCreateOrUpdateConfig}>
            <DialogTitle>{configPartnerairChoice.x_user.trim().length > 2 ? "Modification": "Création" } de la configuration</DialogTitle>
            <DialogContent>
              <DialogContentText sx={{ mb: 3 }}>
                Veuillez faire entrer les informations du compte
              </DialogContentText>
