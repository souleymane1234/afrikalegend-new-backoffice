...
export default function CreateActualityView() {
  ...
  const submit = async () => {
    if (editorRef.current) {
      ...
      if(title.trim().length > 3 && fileList.length > 0 && textEditor.toString().length > 12 && author.trim().length > 3 && location.trim().length > 3 && dateFormation.length === 2) {
        setButtonIsLoading(true);
        alert({type: 'loading', content: 'Création en cours...'});
        const [startDate, endDate] = dateFormation;

        const newFormation = await ConsumApi.createFormation({cover:base64, instructor:author.trim(), coverName: nameFileUploadBase64.trim(),description:textEditor.trim(),location, title, startDate:startDate.toISOString(), endDate:endDate.toISOString()});
      ...
      <Stack alignItems="center" justifyContent="center" sx={{ height: 1 }}>
        ...
        >
          ...
          <Stack direction="row" alignItems="center" justifyContent="center" mb={1}  sx={{width: '100%'}}>
            ...
            <TextField name="name" label="Titre de la formation" value={title} onChange={(event) => changeTitle(event.target.value)} />
            <TextField
              name='location'
              
              value={location} onChange={(event) => changeLocation(event.target.value)}
              id="outlined-multiline-flexible"
              label="Lieu de la formation"
            />
            <RangePicker value={dateFormation} onChange={(value, valueString) => {
              setDateFormation(value)
            }} size='large' placeholder={["Date de début", "Date de fin"]} />
            <TextField name="authorName" label="Nom de l'auteur de la formation" value={author}  onChange={(event) => changeAuthor(event.target.value)} />
            <Editor
